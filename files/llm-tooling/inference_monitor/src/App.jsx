...
function mockSnapshot(prev) {
  ...
  return {
    ...
    resources: { cpu_percent: jitter(61, 0.12), mem_mb: jitter(4820, 0.05), mem_limit_mb: 8192 },
    cost: { mips: jitter(325, 0.08), ziip_percent: jitter(82, 0.04), four_hour_avg_mips: jitter(301, 0.06) },
    stability: { error_rate: jitter(0.4, 0.5), timeout_rate: jitter(0.08, 0.6), uptime_sec: (prev?.stability?.uptime_sec ?? 86400) + 5 },
    operability: { startup_ms: 2340, last_restart: "2h 14m ago", log_lines_per_min: jitter(42, 0.2) },
    security: { namespace: "llm-prod", egress: "zos-only", service_account: "llm-infer-sa", token_age_sec: jitter(412, 0.02), token_limit_sec: 600 },
    integration: { zos_rest: { success_rate: jitter(99.2, 0.005), avg_latency_ms: jitter(18, 0.15), timeout_count: Math.floor(jitter(1, 1)) } },
    health: { status: "healthy", checks: { inference: true, zos: true, iam: true, memory: true } },
  };
}

function fmt(n, dec = 1) { return typeof n === "number" ? n.toFixed(dec) : "—"; }
function fmtUptime(sec) { const h = Math.floor(sec / 3600); const m = Math.floor((sec % 3600) / 60); return `${h}h ${m}m`; }
...
function MiniBar({ value, max, color = "var(--accent)" }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
      ...
      <span style={{ fontSize: "0.65rem", color: warn ? "var(--yellow)" : "var(--text2)", minWidth: "2.5rem", textAlign: "right" }}>{fmt(pct, 0)}%</span>
    </div>
  );
}
...
function Dot({ ok, label }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "0.45rem", marginBottom: "0.35rem" }}>
      <div style={{ width: 7, height: 7, borderRadius: "50%", background: c, boxShadow: `0 0 6px ${c}`, animation: ok ? "pulse 2s infinite" : "none" }} />
      <span style={{ fontSize: "0.68rem", color: "var(--text)" }}>{label}</span>
      <span style={{ marginLeft: "auto", fontSize: "0.6rem", fontWeight: 600, color: c }}>{ok ? "OK" : "FAIL"}</span>
    </div>
  );
}
...
function TokenArc({ age, limit }) {
  ...
  return (
    <svg width={56} height={56}>
      ...
      <text x={cx} y={cy + 4} textAnchor="middle" fill={color} style={{ fontSize: "9px", fontFamily: "var(--mono)", fontWeight: 600 }}>{fmt(age, 0)}s</text>
    </svg>
  );
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const update = () => {
      // To connect to a real backend replace mockSnapshot() with:
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root {
          --bg: #080b0f; --surface: #0d1117; --surface2: #111820; --border: #1a2535;
          --accent: #00c8ff; --green: #39d98a; --yellow: #f5a623; --red: #ff4d6a;
          --purple: #b57bee; --orange: #ff7c43; --teal: #00e8c6;
          --text: #c9daea; --text2: #4a6278;
      ...
      <div style={{ minHeight: "100vh", background: "var(--bg)", padding: "1.25rem" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.25rem", paddingBottom: "1rem", borderBottom: "1px solid var(--border)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "1.5rem" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", background: allHealthy ? "rgba(57,217,138,0.08)" : "rgba(255,77,106,0.08)", border: `1px solid ${allHealthy ? "rgba(57,217,138,0.25)" : "rgba(255,77,106,0.25)"}`, borderRadius: 6, padding: "0.4rem 0.8rem" }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: allHealthy ? "var(--green)" : "var(--red)", boxShadow: `0 0 8px ${allHealthy ? "var(--green)" : "var(--red)"}`, animation: "pulse 2s infinite" }} />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "0.9rem" }}>
          ...
          <Card title="Latency" category="Performance" accent="var(--accent)">
            ...
            <div style={{ marginTop: "0.6rem" }}><Sparkline history={history.avg} color="var(--accent)" /></div>
            <div style={{ fontSize: "0.58rem", color: "var(--text2)", marginTop: "0.3rem", fontFamily: "var(--mono)" }}>30-point avg_ms trace</div>
          </Card>
          ...
          <Card title="Throughput" category="Throughput" accent="var(--teal)">
            <div style={{ display: "flex", alignItems: "flex-end", gap: "0.4rem", marginBottom: "0.5rem" }}>
              <span style={{ fontFamily: "var(--mono)", fontSize: "2rem", fontWeight: 600, color: "var(--teal)", lineHeight: 1 }}>{fmt(inference.throughput_rps)}</span>
            ...
            <div style={{ marginTop: "0.6rem" }}>
              <Row label="Error rate" value={fmt(inference.errors_perc, 2)} unit="%" accent={inference.errors_perc > 1 ? "var(--red)" : "var(--green)"} />
            </div>
          </Card>
          ...
          <Card title="Resource Use" category="Resources" accent="var(--purple)">
            ...
            <div style={{ marginTop: "0.6rem" }}><Sparkline history={history.cpu} color="var(--purple)" height={28} /></div>
          </Card>
          ...
          <Card title="Cost Signal" category="z/OS Cost" accent="var(--yellow)">
            ...
            <div style={{ marginTop: "0.5rem" }}><MiniBar value={cost.ziip_percent} max={100} color="var(--teal)" /></div>
            <div style={{ marginTop: "0.5rem" }}><Sparkline history={history.mips} color="var(--yellow)" height={28} /></div>
          </Card>
          ...
          <Card title="Stability" category="Error & Timeout" accent="var(--red)">
            ...
            <div style={{ marginTop: "0.5rem", display: "flex", gap: "0.5rem" }}>
              {[0.5, 1, 2, 3, 4].map((thresh, i) => (
                <div key={i} style={{ flex: 1, height: 24, borderRadius: 3, background: stability.error_rate < thresh ? "rgba(57,217,138,0.1)" : "rgba(255,77,106,0.1)", border: `1px solid ${stability.error_rate < thresh ? "rgba(57,217,138,0.2)" : "rgba(255,77,106,0.2)"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.55rem", fontFamily: "var(--mono)", color: "var(--text2)" }}>
                  {thresh}%
                </div>
              ))}
            </div>
          ...
          <Card title="Operability" category="Runtime Health" accent="var(--orange)">
            ...
            <div style={{ marginTop: "0.75rem", paddingTop: "0.75rem", borderTop: "1px solid var(--border)" }}>
              <div style={{ fontSize: "0.62rem", color: "var(--text2)", marginBottom: "0.4rem", fontFamily: "var(--mono)", letterSpacing: "0.06em" }}>HEALTH CHECKS</div>
          ...
          <Card title="Security" category="IAM & Network" accent="var(--green)">
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
              ...
              <div style={{ marginLeft: "0.75rem" }}>
                ...
                <div style={{ fontSize: "0.55rem", color: "var(--text2)", textAlign: "center", marginTop: "0.2rem", fontFamily: "var(--mono)" }}>TOKEN AGE</div>
              </div>
            </div>
            <div style={{ marginTop: "0.6rem", paddingTop: "0.6rem", borderTop: "1px solid var(--border)" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "0.35rem", background: "rgba(57,217,138,0.08)", border: "1px solid rgba(57,217,138,0.2)", borderRadius: 4, padding: "0.25rem 0.6rem", fontSize: "0.62rem", fontFamily: "var(--mono)", color: "var(--green)" }}>
          ...
          <Card title="z/OS Integration" category="REST Reliability" accent="var(--teal)">
            <Row label="Success Rate" value={fmt(integration.zos_rest.success_rate, 2)} unit="%" accent={integration.zos_rest.success_rate > 99 ? "var(--green)" : "var(--yellow)"} />
            <Row label="Avg Latency" value={fmt(integration.zos_rest.avg_latency_ms)} unit="ms" accent="var(--teal)" />
            <Row label="Timeouts" value={integration.zos_rest.timeout_count} accent={integration.zos_rest.timeout_count > 0 ? "var(--yellow)" : "var(--green)"} />
            <div style={{ marginTop: "0.75rem" }}>
              <div style={{ height: 6, borderRadius: 3, overflow: "hidden", background: "var(--border)" }}>
                <div style={{ width: `${clamp(integration.zos_rest.success_rate, 0, 100)}%`, height: "100%", background: "linear-gradient(90deg, var(--teal), var(--green))", borderRadius: 3, transition: "width 0.6s ease", boxShadow: "0 0 8px var(--teal)" }} />
              </div>
